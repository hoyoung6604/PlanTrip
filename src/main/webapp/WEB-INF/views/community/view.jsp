...
<head>
  ...
  <style>
    ...
    .rp-uploader-row input[type="file"]{ width:min(460px, 100%); font-size:12px; }
    .rp-upload-btn{
      padding:8px 12px; font-size:13px; border-radius:12px;
    ...
    .rp-post{
      ...
      margin-bottom:14px; max-width: 980px;
    }

    .rp-photo-box{
      width:360px; aspect-ratio: 16 / 9; height:auto;
      border-radius:14px; overflow:hidden; background:rgba(255,255,255,.02);
      border:1px solid rgba(255,255,255,.10);
      flex:0 0 auto; display:flex; align-items:center; justify-content:center;
    }
    .rp-photo-box img{ width:100%; height:100%; object-fit:cover; display:block; }

    .rp-text{
      flex:1 1 auto; min-width:220px;
      display:flex; flex-direction:column;
    }
    .rp-text .rp-caption-label{ font-size:12px; color:rgba(255,255,255,.55); margin-bottom:8px; }
    .rp-text textarea{
    ...
    .rp-delbtn{
      ...
      border:1px solid rgba(255,255,255,.14);
      background:rgba(255,0,0,.08); cursor:pointer;
    }
    ...
    .rp-modal-overlay{
      ...
      z-index:999999; transform:none !important;
    }
    .rp-modal{
      width:min(420px, 92vw); border-radius:16px;
      border:1px solid rgba(255,255,255,.14);
      background:rgba(20,23,32,.98); padding:16px;
    }
    .rp-modal h4{ margin:0 0 8px; font-size:16px; }
    .rp-modal p{ margin:0 0 14px; font-size:13px; color:rgba(255,255,255,.65); }
    .rp-modal-actions{ display:flex; justify-content:flex-end; gap:8px; }
    .rp-btn{
      padding:8px 12px; font-size:13px; border-radius:12px;
      border:1px solid rgba(255,255,255,.16); background:rgba(255,255,255,.06);
      cursor:pointer;
    }
    .rp-btn-primary{ background:rgba(0,170,255,.18); border-color:rgba(0,170,255,.28); }
      ...
      <div class="mp-card-head">
        <div>
		  ...
		  <div class="mp-card-sub">
		    ...
		    작성자:
		    <c:out value="${review.member.MName}" />

		    &nbsp;|&nbsp; 장소:
		    <c:out value="${empty review.spot or empty review.spot.city ? '-' : review.spot.city.name}" />
		  </div>
                    ...
                    <c:if test="${not empty sessionScope.loginMember and sessionScope.loginMember.MIdx == review.MIdx}">
                      <div class="rp-actions">
                        <form action="${pageContext.request.contextPath}/review-photos/delete"
                          ...
                          <input type="hidden" name="rvIdx" value="${review.rvIdx}">
                          <button type="submit" class="rp-delbtn">삭제</button>
                        </form>
                      </div>
                    </c:if>
                  </div>
...
(function(){
  ...
  function hasFiles(){ return fileInput.files && fileInput.files.length > 0; }
  function openModal(){
  ...
  yesBtn.addEventListener('click', function(){ closeModal(); form.submit(); });
