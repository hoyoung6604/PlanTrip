...
<head>
  ...
  <!--welcome to the PlanTrip 폰트임-->
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/ui-toast.css" />

    <script defer src="${pageContext.request.contextPath}/js/ui-toast.js"></script>
    <script defer src="${pageContext.request.contextPath}/js/theme.js"></script>
    <script defer src="${pageContext.request.contextPath}/js/auth-modal.js"></script>
    <script defer src="${pageContext.request.contextPath}/js/auth-guard.js"></script>
    <script defer src="${pageContext.request.contextPath}/js/pages/index.js"></script>
    <script defer src="${pageContext.request.contextPath}/js/scrollbar-auto.js"></script>
    <script defer src="${pageContext.request.contextPath}/js/header-scroll.js"></script>
...
<header class="header" id="header">
  <div class="container header-inner">
    ...
    <nav class="nav">
      ...
      <!-- 로그인 상태에서는 nav 라인에 마이페이지 링크도 함께 노출(줄바꿈/깨짐 방지) -->
      ...
      fetch('/spots/api/recommend?category=' + catCode)
          ...
          .then(data => {
  			...
  			data.forEach(spot => {
  				...
  				// 기존 card.innerHTML 부분 교체
  				card.innerHTML = 
  				    '<button class="wish-btn" onclick="toggleWish(event, ' + spot.id + ', this)" style="position: absolute; top: 15px; right: 15px; z-index: 10; background: rgba(255,255,255,0.8); border: none; border-radius: 50%; width: 35px; height: 35px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 18px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">' + (spot.isHearted ? '❤️' : '🤍') + '</button>' +
